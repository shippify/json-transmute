...
function resolve(expr, scope, rootScope, isKey) {
	...
	tokens.forEach(function (token) {
		...
		switch (token.type) {
			case "scope":
				childScope = resolve(token.val, token.root ? rootScope : scope, rootScope).val;
				result = "";
				break;
			case "lookup":
				...
				break;
			case "expression":
				result = resolve(token.val, token.root ? rootScope : childScope, rootScope, isKey).val;
				break;
			case "filter":
				result = filter(token.val, token.opts, childScope, rootScope, childScope);
				break;
			case "static":
				result = token.val;
				break;
...
function lookup(key, result) {
	(key || "").split(".").some(function (key) {
		...
		if (
			result &&
			((Array.isArray(result) && intKey in result) || (typeof result === "string" && intKey < result.length))
		) {
...
function parse(expr) {
	...
	while (i < expr.length) {
		...
		switch (expr[i]) {
			...
			case "(":
				level.push(")");
				if (level.length === 1 && ((addFilter && filter.val !== "") || (!addFilter && token.val !== ""))) {
			...
			case " ":
				...
				if (addOpts && level.length === 1 && (addFilter || token.type === "filter")) {
					++i;
					continue;
				}
				break;
